...
public class LocalImageHelper implements Serializable{
    ...
    public String setCameraImgPath() {
        String foloder= getCachePath() + "/PostPicture/";
...
//            return;
//
//        LogUtils.jolie("aaaaaaaaaaaaaaaaaaaaaaaaa1111");
//
//        isRunning=true;
        if (isInited())
            return;
//
        ...
        Cursor cursor = context.getContentResolver().query(
                ...
                null,         // No where clause
                MediaStore.Images.Media.DATE_TAKEN + " ASC"); //根据时间升序
        if (cursor == null)
            return;
    ...
    public String getCachePath() {
        ...
        if (android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED))
            cacheDir = CommonUtil.getInstance().getContext().getExternalCacheDir();
        else
            cacheDir = CommonUtil.getInstance().getContext().getCacheDir();
        if (!cacheDir.exists())
            cacheDir.mkdirs();
